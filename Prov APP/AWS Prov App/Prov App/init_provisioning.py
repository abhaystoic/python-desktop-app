...
calling certificate fetching script and update the certificate on AWS, validating the data received and calling Symportal's REST API for 
...
class App:
  ...
  def call_script(self, root, username, password):
    try:
        ...
        if data[0] == constants.FAILURE_MSG_FROM_FETCH or data[1] == constants.FAILURE_MSG_FROM_FETCH:
          ...
          if cert_validation is True:
            ...
            if response.status_code == 200:
              ...
              #self.message3['text'] = response.status_code
      ...
      self.message3['text'] = ""
      if response is None:
        self.message2['text'] = constants.SERVER_ISSUE
        self.message2['fg'] = 'red'
        self.message3['text'] = ""  
      self.button.config(state='active')
      self.slogan.config(state='active')
